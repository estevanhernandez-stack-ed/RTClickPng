...
- **I abandoned the Settings UI at the wrong layer.** Both WinUI 3 and WPF exited silently during packaged-app activation — no WER, no crash log, no Application Error event. I kept retreating through the tech stack (WinUI 3 → WPF → shell-extension-spawned → Notepad-on-settings.json) and eventually recommended shipping without a real UI.
  - **The actual root cause was `<InvariantGlobalization>true</InvariantGlobalization>` in `Directory.Build.props`**, which I added during item 2 to shrink the Native AOT Engine binary. WPF's `PresentationFramework` requires ICU at startup and fails silently without it — before any .NET exception handler is wired up, which is why there was no log anywhere. Commit `300d0b0 fix(settings): disable InvariantGlobalization for WPF` from the other agent fixed it in one line.
  - **Earlier version of this retro pinned the blame on Partner Center identity / cert trust** (commit `7269d18`). That was wrong — the identity swap happened later and was about Store submission. The order of commits between the handoff and ship is: `300d0b0` (real fix) → `1225659` (revert my Notepad hack, no longer needed) → Store-submission polish → `7269d18` (identity swap) → ship. I latched onto the fancier diagnosis. Correcting the record.
  - **What I should have done:** walked the build-config chain before rewriting the UI. `Directory.Build.props` flags cascade silently to every csproj added later; a `true` I set for the Engine poisoned the WPF app hours later.
